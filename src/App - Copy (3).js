...
function App() {
    ...
    const handleSummarizeEmail = async (e, emailId) => {
        ...
        // Expand the email to show the loading state
        setExpandedEmailId(emailId);
        try {
            ...
            // 1. More robust logic to select the summary string
            let summaryToDisplay = '<p><em>Summary not available.</em></p>'; // Default to valid HTML

            // Prioritize the HTML summary sent from the backend
            // Your Python code sends it as 'summary' or 'html_output' depending on the route
            if (responseData.summary && typeof responseData.summary === 'string') {
                summaryToDisplay = responseData.summary;
            } else if (responseData.html_output && typeof responseData.html_output === 'string') {
                summaryToDisplay = responseData.html_output;
            }

            console.log('Summary to display:', summaryToDisplay)    
            // 2. Update the state
            ...
            setEmails(currentEmails =>
                currentEmails.map(mail =>
                    mail.id === emailId
                        ? {
                            ...mail,
                            summary: summaryToDisplay, // Assign the clean HTML string
                            isSummaryLoaded: true      // Add a flag to indicate completion
                        }
                        : mail
                )
            );
                                    ...
                                    <div key={mail.id} className="bg-white rounded-lg border transition-shadow duration-200 hover:shadow-md" >
                                        ...
                                        {expandedEmailId === mail.id && (
                                            <div className="p-4 ml-4 my-1 border-t">
                                                {summarizingId === mail.id && !mail.summary && (
                                                    <div className="flex items-center text-blue-600"><LoaderIcon className="mr-2"/>Generating AI Summary...</div>
                                                )}
                                                {/* {mail.summary && (
                                                    <div className="p-4 mb-4 bg-blue-50 border-l-4 border-blue-400">
                                                        <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(mail.summary) }} />
                                                    </div>
                                                )} */}
                                                {mail.summary && (
                                                    <div className="p-4 mb-4 bg-blue-50 border-l-4 border-blue-400">
                                                        {/*
                                                        By removing the "prose" className from the div below, we allow our
                                                        custom styles from App.css to take full control. The 'summary-container'
                                                        class within the HTML string will now be styled correctly.
                                                        */}
                                                        <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(mail.summary) }} />
                                                    </div>
                                                )}
                                                {mail.body ? (
