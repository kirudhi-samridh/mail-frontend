                            ...
                            <div className="space-y-1">
                                {emails.length > 0 ? emails.map((mail) => (
                                    <div key={mail.id} className="bg-white rounded-lg border transition-shadow duration-200 hover:shadow-md" >
                                        ...
                                        {/* This is the clickable email header part, which is correct */}
                                        <div className="grid grid-cols-12 gap-4 items-center p-3 cursor-pointer" onClick={() => handleReadEmail(mail.id)}>
    
                                            {/* This part for the sender's name is unchanged */}
                                            <div className="col-span-3 font-medium text-gray-800 truncate">
                                                {mail.from.split('<')[0].trim()}
                                            </div>

                                            {/* --- CHANGES ARE HERE --- */}
                                            {/* This div now uses Flexbox to align the new tag and the subject */}
                                            <div className="col-span-6 text-gray-600 truncate flex items-center gap-2">
                                                {/* The new ClassificationTag component is added.
                                                It uses optional chaining (?.) so it won't cause an error 
                                                if 'summaryJson' doesn't exist yet.
                                                */}
                                                <ClassificationTag classification={mail.summaryJson?.classification} />
                                                
                                                {/* The subject is wrapped in a span for proper alignment */}
                                                <span>{mail.subject}</span>
                                            </div>
                                            {/* --- END OF CHANGES --- */}

                                            {/* This part for the date and summarize button is unchanged */}
                                        ...
                                        {expandedEmailId === mail.id && (
                                            <div className="p-4 ml-4 my-1 border-t space-y-6">
                                                ...
                                                {/* --- SECTION 1: ORIGINAL EMAIL (Always visible and full-width when expanded) --- */}
                                                ...
                                                {mail.isSummaryLoaded && mail.summaryBreakdown && (
                                                    <div className="summary-accordion-group">
                                                        <div className="flex justify-between items-center mb-2">
                                                            ...
                                                            {mail.summaryJson?.audioScript && (
                                                                ...
                                                                >
                                                                    {/* The icon changes based on the 'isSpeaking' state */}
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return (
                                                                    <div className="accordion-container space-y-2">
                                                                        {/* --- Custom Accordion Item for Classification (with full content) --- */}
                                                                        {classification && classification.category && (
                                                                            <details 
                                                                                key="classification" 
                                                                                className="group border rounded-lg overflow-hidden mb-2 bg-white shadow-sm"
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <div className="p-4 border-t text-sm space-y-2">
                                                                                    {classification.confidenceScore && (
                                                                                        <div>
                                                                                            <strong>Confidence:</strong> {Math.round(classification.confidenceScore * 100)}%
                                                                                        </div>
                                                                                    )}
                                                                        ...
                                                                        {/* --- Accordion for all other summary parts --- */}
                                                                        {Object.keys(mail.summaryBreakdown)
